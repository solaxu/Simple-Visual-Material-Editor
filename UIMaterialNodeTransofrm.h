...
{
	class UINodeTransform : public SUIWindow
	{
	public:
		UINodeTransform(float x, float y, float width, float heigth, UIObject* parent)
			: SUIWindow(x, y, width, heigth, UIDrawParams::DefaultBorderWidth, parent)
		{
			...
			m_nodeType->Name = CString(_T("Transform"));
			m_nodeType->ShowBorder(TRUE);

			m_body = new SUILabel(x, y + heigth * 2, width, 80, UIDrawParams::DefaultBorderWidth, this);
